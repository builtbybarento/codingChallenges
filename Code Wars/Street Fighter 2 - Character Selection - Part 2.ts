// 6 kyu - Street Fighter 2 - Character Selection - Part 2
...
export function superStreetFighterSelection(fighters: string[][], position: [number,number], moves: string[]): string[] {
  ...
  moves.forEach(move=>{
    console.log(move)
    if(move==="up" && position[0] > 0 && fighters[position[0] -1 ][position[1]] !== "" ) position[0] --
    if(move==="down" && position[0] < fighters.length - 1 && fighters[position[0] + 1][position[1]] !== "") position[0] ++
    if (move==='right') position[1] === fighters[position[0]].length -1 ? position[1] = 0 : position[1] += 1
    if (move==='left') position[1] === 0 ? position[1] = fighters[position[0]].length -1 : position[1] -= 1
...
//so now we do the same thing but also must also consider if the next space is empty 
//if current[0] != 0, and fighters[current[0] -1 ][current[1]] !== "" 

//need a while loop for the left and right conditions